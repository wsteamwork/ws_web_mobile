...
import { Avatar, Box, Divider, Grid, IconButton, List, ListItem, ListItemSecondaryAction, ListItemText, Theme, Typography } from '@material-ui/core';
...
const SettingInApp: FC<IProps> = (props) => {
  ...
  // console.log(profile);
  // const imgAvatar = isLogin && profile && profile.avatar ? `${IMAGE_STORAGE_SM}${profile.avatar}` : '../../static/images/westay-avatar.jpg';
          ...
          <Grid item xs={8}>
            {isLogin ? (
              <Fragment>
                ...
                <Typography variant='body1'>{t('home:editProfile')}</Typography>
              </Fragment>
            ) : (
                <Fragment>
                  <Typography variant='h5' className={classes.txtName} onClick={() => { router.push('/auth') }}>{t('home:signUp/signIn')}</Typography>
                  <Typography variant='body1'>{t('home:letSignUp')}</Typography>
                </Fragment>
              )}
          </Grid>
