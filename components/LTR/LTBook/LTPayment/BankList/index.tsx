...
import { LTredirectToBaoKim } from '@/store/Redux/Reducers/LTR/LTBooking/ltbooking';
import '@/styles/components/shared/payment_hover.scss';
import { BaoKimBankInfo, PaymentBankListRes, PaymentMethod } from '@/types/Requests/Payment/PaymentResponse';
...
const BankList: FC = (props) => {
  ...
  const payment_methods = useSelector<ReducersList, PaymentMethod[]>(
    (state) => state.ltBooking.LTDataInvoice.bank_list
  );
  ...
  const triggerPayment = async () => {
    if (bankId !== 0) {
      ...
      try {
        const url = await LTredirectToBaoKim(lists.contracts.data[0].uuid, bankId);
